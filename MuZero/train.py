...
def prepare_targets(targets: [[(float, float, int, [float], bool)]], action_space_size: int):
...
def test_prepare_targets():
    ...
    one_hot_actions, search_policies, value_targets, observed_rewards, isNotDone = prepare_targets(targets, 4)
...
def train_on_batch(
    ...
    logger.add_head_losses(loss.item(), pl/num_unroll_steps, vl/num_unroll_steps, rl/num_unroll_steps)
