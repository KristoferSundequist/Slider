...
def get_episode(num_iterations: int, epsilon: float):
    with torch.no_grad():
        ...
        for i in range(num_iterations):
            qvalues = agent.forward(torch.tensor([current_state]))
            ...
            values.append(qvalues[0][action].item())
...
def pvo(trainingData: TrainingData):
    for _ in range(globals.pvo_epochs):
        ...
        for indices in sampler:
            taken_actions = trainingData.one_hot_actions[indices]

            all_new_values = agent.forward(trainingData.states[indices])
            assert all_new_values.size() == taken_actions.size(), f"{all_new_values.size()} == {taken_actions.size()}"
            new_values = torch.sum(all_new_values * taken_actions, 1)
...
def agent_loop(iterations):
    with torch.no_grad():
        ...
        for i in range(iterations):
            values = agent.forward(torch.tensor([game.get_state()]))
            action = int(values.argmax().item())
            _, _ = game.step(action)

            game.render(win)
            value = str(values[0][action].item())
